...
/* Function prototype*/
double simpson(double low, double high, double x, int n, double width);
...
double simpson(double low, double high, double x, int n, double width)
{
	...
	integral = (select_part(x,low) + 4*select_part(x,low+width) + select_part(x, high));
	lowLimit = lowLimit + width; // adding the first integral
 	
	for (i = 2; i < n; i+=2)
	{
		lowLimit = lowLimit + width;
		integral += 2*select_part(x, lowLimit);
		lowLimit = lowLimit + width;
		integral += 4*select_part(x, lowLimit);
	}
...
void main(int argc, char** argv)
{
	...
	for(j=0; j<n_global; j++) {
		...
		integral_local = simpson(range[0], range[1], xj, n_local, w_local);
